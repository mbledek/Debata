...
class ToFirstModal(ui.Modal):
    def __init__(self):
        super().__init__(
            title=f"Weryfikacja pytania",
            timeout=3600
        )
    ...
    async def callback(self, interaction):
        text = f'**Dzięki wielkie, przesłaliśmy Twoje pytanie do naszych moderatorów!**'
        await interaction.response.send_message(text, ephemeral=True)


class ToSecondModal(ui.Modal):
    def __init__(self):
        super().__init__(
            title=f"Weryfikacja pytania",
            timeout=3600
...
class ToAllModal(ui.Modal):
    def __init__(self):
        super().__init__(
            title="Weryfikacja pytania",
            timeout=3600
        )
